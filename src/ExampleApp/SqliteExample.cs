...
public class TestUser
{
	...
	public TestUser(string username, string discriminator, string firstname, string lastname)
	{
		UserName = username;
...
public class SqliteExample
{
	...
	{
		//Cache your queries!
		var insertQuery = _query.Insert<TestUser>();
		var upsertQuery = _query.Upsert<TestUser>();
		var allQuery = _query.Select<TestUser>();
		var selectQuery = _query.Select<TestUser>(c => c.Prop(t => t.LastName));

		//Setup some data to use
		var john = new TestUser("test", "0001", "Jxhn", "Doe");
		var users = new[]
		{
			john,
			new TestUser("test", "0002", "Jane", "Doe"),
			new TestUser("test", "0003", "Jane", "Smith"),
			new TestUser("some", "0001", "Joe", "Smith")
		};
		...
		var allSmiths = await _sql.Get<TestUser>(selectQuery, new { LastName = "Smith" });
		...
		var allUsers = await _sql.Get<TestUser>(allQuery);
	...
	public static SqliteExample Setup()
	{
		...
		return new ServiceCollection()
			.AddSqlService(c =>
			{
				...
				{
					f.OnInit((con) => con.ExecuteAsync(TestUser.CREATE_TABLE));
				})
				.ConfigureGeneration(c => c.WithCamelCaseChange())
				.ConfigureTypes(c => c.CamelCase().Entity<TestUser>());
			})
